> **Documento histórico da série v9.x.** A v10.1 removeu a autenticação por token do Admin conforme solicitação do projeto.
...
| Segredo do Admin no histórico da v9.x | ℹ️ **Item histórico** — a v10.1 não usa mais autenticação por token no Admin |
...
> ℹ️ Registro histórico: a v9.x utilizava token no Admin. Esse mecanismo foi removido na v10.1 e esta recomendação não se aplica à versão atual.
...
- Na v9.x, o token Admin era mantido em `sessionStorage`. **Na v10.1 esse mecanismo foi removido**; dados do cliente continuam em `sessionStorage` e a sacola em `localStorage`.

---

## 4. Pendências (em ordem de prioridade)

1. **[Encerrado na v10.1] Autenticação antiga do Admin**: o fluxo por token foi removido. O atalho atual de 3 cliques é apenas conveniência e não constitui proteção de acesso.
...
| Higiene de segredos | F | **Histórico v9.x** (Admin atual não usa token) |

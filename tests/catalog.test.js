...
test('catálogo normaliza e não preserva segredos ou campos arbitrários', () => {
  // o seed pode crescer (catálogos PDF); a invariant real e que o fixture
  // continue normalizando sem duplicar ids/nomes e sem perder produtos
  assert.ok(catalog.products.length >= 5, 'catálogo de exemplo deve ter ao menos 5 produtos');
  assert.equal(new Set(catalog.products.map((p) => p.id)).size, catalog.products.length);
  assert.equal(new Set(catalog.products.map((p) => p.name)).size, catalog.products.length);
...
test('catálogo aceita imagens enviadas pelo painel em /media/products/', () => {
  const next = normalizeCatalog({...raw, products: raw.products.map((product, index) => index ? product : {...product, images: ['/media/products/product-image-123-abc']})});
  assert.equal(next.products[0].images[0], '/media/products/product-image-123-abc');
});

test('variação pode apontar para uma foto específica da galeria do produto', () => {
  const product = structuredClone(raw.products.find(item => (item.variants || []).length));
  product.images = ['/assets/products/variante-capa.webp', '/assets/products/variante-1l.webp'];
  product.variants[0].image = product.images[1];
  const next = normalizeCatalog({...raw, products: [product]});
  assert.equal(next.products[0].variants[0].image, '/assets/products/variante-1l.webp');
  assert.equal(publicCatalog(next, baseConfig).products[0].variants[0].image, '/assets/products/variante-1l.webp');
});

test('foto da variação precisa pertencer à galeria do produto', () => {
  const product = structuredClone(raw.products.find(item => (item.variants || []).length));
  product.images = ['/assets/products/variante-capa.webp'];
  product.variants[0].image = '/assets/products/foto-fora-da-galeria.webp';
  const next = normalizeCatalog({...raw, products: [product]});
  assert.equal(next.products[0].variants[0].image, '');
});
...
test('catálogo público sincroniza modo/valores de frete do servidor', () => {
  ...
  // O modo público permite a cotação mesmo se /api/health estiver indisponível.
  const correiosMode = publicCatalog(catalog, {...baseConfig, shippingMode: 'correios'});
  assert.equal(correiosMode.settings.shipMode, 'correios');
...
test('produto oculto permanece no Admin, não aparece no catálogo público e não pode ser comprado', () => {
  const hiddenId = raw.products[0].id;
  const hiddenCatalog = normalizeCatalog({
    ...raw,
    products: raw.products.map((product, index) => index === 0 ? {...product, hidden: true} : product)
  });
  assert.equal(hiddenCatalog.products.find(product => product.id === hiddenId)?.hidden, true);
  const published = publicCatalog(hiddenCatalog, baseConfig);
  assert.equal(published.products.some(product => product.id === hiddenId), false);
  assert.throws(() => buildOrder({
    customer: {name: 'Cliente', email: 'cliente@example.com'},
    shipping: {choice: 'pickup'},
    items: [{productId: hiddenId, qty: 1}]
  }, hiddenCatalog, baseConfig), /não está mais disponível/);
});

test('configuração visual preserva controles do editor visual e texto do rodapé', () => {
  const next = normalizeCatalog({
    ...raw,
    settings: {
      ...raw.settings,
      footerDescription: 'Descrição editada no construtor visual.',
      visual: {
        ...(raw.settings?.visual || {}),
        visibility: {
          ...(raw.settings?.visual?.visibility || {}),
          headerBrand: false,
          filterBlock: false,
          products: true,
          footerLogo: false
  ...
  assert.equal(next.settings.footerDescription, 'Descrição editada no construtor visual.');
  assert.equal(next.settings.visual.visibility.headerBrand, false);
  assert.equal(next.settings.visual.visibility.filterBlock, false);
  assert.equal(next.settings.visual.visibility.products, true);
  assert.equal(next.settings.visual.visibility.footerLogo, false);
});

test('IDs de variação precisam ser únicos em todo o catálogo', () => {
  assert.throws(() => normalizeCatalog({
    version: 11,
    settings: {},
    commerce: {},
    products: [
      {id: 'p-global-a', name: 'Produto A', price: 1000, available: true, variants: [{id: 'variant-global', name: 'A', price: 1000}]},
      {id: 'p-global-b', name: 'Produto B', price: 1200, available: true, variants: [{id: 'variant-global', name: 'B', price: 1200}]}
    ]
  }), /ID de variante duplicado no catálogo/);
});

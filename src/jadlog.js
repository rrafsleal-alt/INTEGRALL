...
export class JadlogService extends ShippingProvider {
  constructor({token, cnpj, conta, contrato, originCep, modalidade = 3, tpEntrega = 'D', tpSeguro = 'N', baseUrl, fetchImpl} = {}) {
    super({providerId: 'jadlog', capabilities: {quote: true, tracking: true}});
  ...
  async quote(cepDestino, pack, declaredCents = 0, {bypassCache = false} = {}) {
    ...
    if (!bypassCache && cached && Date.now() - cached.at < QUOTE_CACHE_TTL_MS) return cached.value;

...
export function evaluatePayment(order, payment) {
  ...
  if (status === 'approved') {
    if (order.inventoryReservationReleasedAt && !order.inventoryCommittedAt) {
      return {shouldUpdate: true, nextStatus: 'payment_review', paymentStatus: 'approved_after_reservation_expired', warning: 'approved_after_reservation_expired'};
    }
  ...
  if (['pending', 'in_process', 'authorized'].includes(status)) {
    if (order.inventoryReservationReleasedAt && !order.inventoryCommittedAt) return {shouldUpdate: false, nextStatus: order.status, paymentStatus: 'reservation_expired'};
    return advanced

...
export class Mailer {
  ...
  async send({to, subject, text, html}) {
    ...
    try {
      ...
      await conn.command('QUIT', [221]).catch(error => {
        if (process.env.NODE_ENV !== 'test') console.warn(`SMTP encerrou sem confirmação: ${String(error?.message || error).slice(0, 200)}`);
      });

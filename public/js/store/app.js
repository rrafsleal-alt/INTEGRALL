(() => {
  ...
  function bindAdminShortcut() {
    const logo = document.querySelector('.footer-secret-brand');
    if (!logo || logo.dataset.adminShortcutBound === '1') return;
    logo.dataset.adminShortcutBound = '1';
    let clicks = 0;
    let timer = null;
  ...
  async function boot() {
    ...
    if (/^https?:$/.test(location.protocol) && globalThis.IntegrallApi) {
      try {
        ...
        const notice = document.getElementById('catalogOfflineNotice'); if (notice) notice.hidden = true;
        ...
        if (Array.isArray(remote.promotions)) app.setPromotions?.(remote.promotions);
        ...
        const notice = document.getElementById('catalogOfflineNotice'); if (notice) notice.hidden = false;
